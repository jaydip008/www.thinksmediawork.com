...
function handleMenuVisibility() {
  const menuOpenClose = document.querySelector('.menu_open_close');

  // Check screen width and apply appropriate display styles
  if (window.innerWidth <= 1024) {
    menuOpenClose.style.display = 'block';
  } else {
    menuOpenClose.style.display = 'none';
  }
}

// Ensure the menu visibility is correct on page load
...
function openNav() {
  ...
  document.body.classList.add('menu_overlap');
}
...
$(document).ready(function() {
  ...
  $(".is-open-a .toggle-icon, .is-open-a > a").click(function(event) {
    event.preventDefault();
    
    // Toggle the 'is-open' class to the parent li
    $(this).closest(".is-open-a").toggleClass("is-open");
